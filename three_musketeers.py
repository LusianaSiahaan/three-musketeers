...
# by David Matuszek and Rui Yang.
...
def choose_computer_move(who):
    ...
    options = all_possible_moves_for(who)
    if who == 'M' and musketeer_last_move:
        computer_musketeer_intelligence = next(filter(lambda x:x[1]!=musketeer_last_move[1], options))
        if computer_musketeer_intelligence:
            return computer_musketeer_intelligence[random.randint(0,len(computer_musketeer_intelligence)-1)]
        return all_possible_moves_for(who)[random.randint(0,len(all_possible_moves_for(who))-1)] 
    if who == 'R' and enemy_last_move:
        computer_enemy_intelligence = next(filter(lambda x:x[1]==enemy_last_move[1], options))
        if computer_enemy_intelligence:
            return computer_enemy_intelligence[random.randint(0,len(computer_enemy_intelligence)-1)]
        return all_possible_moves_for(who)[random.randint(0,len(all_possible_moves_for(who))-1)]
    return all_possible_moves_for(who)[random.randint(0,len(all_possible_moves_for(who))-1)]
...
def describe_move(who, location, direction):
    ...
    new_location = adjacent_location(location, direction)
    if who == 'M':
        musketeer_last_move = (location,direction)
    if who == 'R':
        enemy_last_move = (location,direction)
